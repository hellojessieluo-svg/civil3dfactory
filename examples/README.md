...
| `pkt/02-two-design-lines.json` | channel-demo | assembly `C3DF-TwoLines` from `pkt/SlopeTop_*.pkt` + stock MarkPoint / LinkToMarkedPoint hooked to the toes, corridor `C1_TwoLines` targeting `C1_L16m` / `C1_R16m`, corridor surface, 23 sample lines, quantities, 23 section views (bottom width follows the lines); writes `tmp/two-lines.dwg`, `tmp/two-lines-section.png` |
