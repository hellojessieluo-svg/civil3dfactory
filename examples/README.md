...
| Design lines | `C1_L16m`, `C1_R16m` | offset alignments 16 m left / right of C1 (`offset_alignment`, style `@C3DF-TopLine`): the top edges for the two-design-lines pattern |
...
| `pkt/02-two-design-lines.json` | channel-demo | assembly `C3DF-TwoLines` from `pkt/SlopeTop_*.pkt` + stock MarkPoint on the right toe + LinkToMarkedPoint from the left toe, corridor `C1_TwoLines` targeting `C1_L16m` / `C1_R16m`, corridor surface, 23 sample lines, quantities, 23 section views (bottom width follows the lines); writes `tmp/two-lines.dwg`, `tmp/two-lines-section.png` |
